...
# Value
area_hectares <- 1.0
# assigns object on right to name on left, i.e. 1.0 goes into area_hectares
# area_hectares can be referred to as a variable
# no output is printed in console -> variable is stored in memory (Environment)
...
r_area
# r_area = 8 #it didn't change - need to rerun line 96
...
# To avoid errors with relative paths, use the here() function
library(here)
interviews <- read_csv(here("data/SAFI_clean.csv"), na = "NULL")
# The here() function takes folder and file names as input
...
n_rows <- nrow(interviews)  # get the number of rows in interviews dataset: 131
...
# Remove the quotes around numbers and logicals and
# make sure data type is the same for each vector.
...
# Filter observations with the "and" operator (comma)
...
interviews %>% 
  group_by(no_meals) %>% 
  summarise(households_2_meals = n()) %>% 
  filter(no_meals == 2)
...
# Let's first check if any months repeat
summary(interviews$interview_date)

# Since no months repeat, we don't need to worry about extracting
# the year AND the month

# Extract the month from the interview_date column
interviews %>%
...
# Remember the 3 rules of defining tidy data:
...
# Let’s transform the interviews data to create new
# columns for each item owned by a household.

# We have multiple items in a single cell, so we need to separate the values
# of items_owned based on the presence of semi-colons (;)

interviews_items_owned <- interviews %>%
  select(key_ID, items_owned) %>% 
  ## separate multiple items in items_owned
...
# This format allows us to do interesting things
# Count number of respondents in each village who owned a bicycle
interviews_items_owned %>%
  filter(bicycle) %>%
  group_by(village) %>%
  count(bicycle)

# Calculate the average number of items owned by respondents in each village
interviews_items_owned %>%
  group_by(village) %>%
...
# Let's clean up our data so that we can use it for plotting.
# Some of the cells in the months_lack_food column contain multiple months,
# which are separated by semi-colons (;).
# Replicate the previous steps with the months_lack_food column in the interviews dataframe.
...
## Plotting data ##
## Start with the interviews_items_owned dataframe
interviews_plotting <- interviews_items_owned %>%
  ## group by key_ID again, since we ungrouped previously
  group_by(key_ID) %>% 
  ## separate multiple items in months_lack_food
...
# Handle this by changing the transparency of the points or
...
# Specify the amount of random motion in the jitter
# When we don’t specify values for width and height,
# geom_jitter() defaults to 40% of the resolution of the data
# (the smallest change that can be measured).
# Pick values between 0.1 and 0.4
interviews_plotting %>%
  ggplot(aes(x = no_membrs, y = number_items)) +
...
# Another way to plot data with overlapping points is to use the geom_count()
interviews_plotting %>%
  ggplot(aes(x = no_membrs, y = number_items, color = village)) +
  geom_count()

# Colour only a subset of values
ggplot(interviews_plotting,
       aes(x = no_membrs,
           y = number_items)) +
  ## First add base colour
  geom_point(color = "grey") +
  geom_point(data = subset(interviews_plotting, village == "Ruaca"),
             color = "red")
...
# This is not a great way to show this type of data because
# it is difficult to distinguish between villages.
# Rather use a boxplot to visualise the distribution of rooms for each wall type.
# . -----------------------------------------------------------------------
...
# Colour only a subset of data
interviews_plotting %>%
  ggplot(aes(x = respondent_wall_type)) +
  geom_bar(fill = "grey") +
  geom_bar(
    data = subset(interviews_plotting, village == "Ruaca"),
    fill = "red"  # overrides the fill aesthetic
  )
