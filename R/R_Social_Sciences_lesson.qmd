...
This lesson is based on the [Social Science Data Carpentry lesson](https://datacarpentry.github.io/r-socialsci/).
...
-   [RStudio `->` a graphical front end (interface) to R, which makes using R a lot nicer]{.medium}

![](images/r-vs-rstudio.png){fig-align="center" width="700"}
...
## What can you do in R? {.smaller .hcenter}

![](images/what-can-you-do-in-r.png){fig-align="center" width="900"}
...
-   Finding more functions and packages
    ...
    -   To search all available packages, use [rdocumentation.org](rdocumentation.org) website
    -   Google `R <task>`
...
-   Where to ask for help:
    ...
    - Generative AI
        - It can be helpful, but not always reliable or just plain wrong
...
-   object name = can be any word you like, but avoid dots (and no spaces!)
...
| Syntax        | Description |
|-----------------|-------------|
| `read_csv("data.csv", skip = 5)` | Skip first *n* lines |
| `read_csv("data.csv", comment = "#")` | Drop lines starting with a given character |
| `read_csv("data.csv", col_names = FALSE)` | No column names; label sequentially X1 to Xn |
| `read_csv("data.csv", col_names = c("name", "surname", "age", "weight"))` | Supply a character vector as column names |
| `read_csv("data.csv", na = ".")` | Specify value(s) representing missing data |
| `read_csv("data.csv", trim_ws = TRUE)` | Trim whitespace |
...
-   Each row contains information about a single interview respondent
-   Each column represents a single variable
-   A description of each variable is available [here](https://datacarpentry.github.io/r-socialsci/02-starting-with-data.html#presentation-of-the-safi-data)
...
x <- factor(c("M", "F", "F", "U", "F", "M", "M", "M", "F", "U"))
levels(x)
```

![](images/factor.png){fig-align="center" width="700"}
...
-   [{dplyr}]{.green} package makes tabular data manipulation easier
-   [{tidyr}]{.green} package enables you to convert between different data formats for plotting and analysis
...
-   Install package once - `install.packages("package_name")`
-   Load package with every R session when you need it - `library(package_name)`
-   [{tidyverse}]{.green} = umbrella package that installs several packages
...
## Reshaping your data: `separate_longer_delim` {.hcenter}

![](images/separate-longer-delim.png){fig-align="center" width="800"}

## Reshaping your data: `replace_na` {.hcenter}

![](images/replace-na.png){fig-align="center"}

## Reshaping your data: `mutate` {.hcenter}

![](images/mutate.png){fig-align="center"}

## Reshaping your data: `if_else` {.hcenter}

![](images/if-else.png){fig-align="center"}

## Reshaping your data: `pivot_wider` {.hcenter}

![](images/pivot-wider.png){fig-align="center"}
